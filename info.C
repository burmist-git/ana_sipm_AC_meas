...
int info(){
  ...
  for(Int_t i = 0;i<nn;i++){
    two_pe = floor(2.0*f_fit->Eval(Vbias[i])*1000.0)/1000.0;
    cout<<setw(10)<<i
        <<setw(10)<<Vbias[i]
        <<setw(15)<<floor(f_fit->Eval(Vbias[i])*1000.0)/1000.0
        <<setw(15)<<floor(f_fit->Eval(Vbias[i])/2.0*1000.0)/1000.0
        <<setw(15)<<two_pe;
...
void read_single_pe_file( TString single_pe_file, TGraphErrors *gr_err){
  ...
  if (file_info.is_open()){
	...
	Int_t i = gr_err->GetN();
	gr_err->SetPoint(i,v,a);
	gr_err->SetPointError(i,v*0.001,a*0.001 + 0.001);
      }
    }
  }
}
